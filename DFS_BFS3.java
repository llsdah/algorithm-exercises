...
public class DFS_BFS3 {
    ...
    public static int solution(String begin, String target, String[] words) {
        ...
        String beginCk = begin;
        String targetCk = target;
        ...
        for ( int i =0 ; i < words.length ; i ++) {
        	...
        	if(  target.equals(checkOne(beginCk, target))) {
        		return answer+1;
        	}
        	
        	beginCk = checkOne(beginCk, words[i]);
        	answer++;
        }
    ...
    // 다른게 1개인 경우만 변환 해서 넘기기 
	private static String checkOne(String beginCk, String words) {
		String re = beginCk;
		int count =0;
		
		for( int i = 0; i<words.length(); i++) {
			
			if(words.charAt(i)!=beginCk.charAt(i)) {
				count ++;
			}
